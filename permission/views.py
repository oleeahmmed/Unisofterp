...
def dashboard_view(request):
    ...
    try:
        ...
        print(f"DEBUG - Total Users: {context['total_users']}")
        print(f"DEBUG - Total Customers: {context['total_customers']}")
        ...
        ).filter(total_sales__isnull=False).order_by('month'))
        ...
        ).filter(revenue__isnull=False).values('name', 'revenue').order_by('-revenue')[:5])
        ...
        ).filter(total_sales__isnull=False, category__isnull=False).order_by('-total_sales')[:5])
        ...
        ).filter(daily_sales__isnull=False).order_by('date'))
        ...
        print(f"DEBUG - Total Sales Orders: {context['total_sales_orders']}")
        ...
        logger.error(f"Unexpected error in dashboard_view: {str(e)}", exc_info=True)
